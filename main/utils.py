...
default_apps_icon = {
    'auth': 'icofont-users',
    'django_cron': 'icofont-wall-clock',
}
...
def get_menu_items(context):
    ...
    custom_app_list = []
    groups = []
    if context['user'].id not in (None, False):
        groups = Group.objects.filter(user=context['user'].id).values_list('id', flat=True)
    parent_items_menu = MainMenu.objects.filter(parent_id__isnull=True).order_by('position')
    if parent_items_menu:
        for parent_item in parent_items_menu:
            custom_items = MainMenu.objects.filter((Q(user=context['user'].id) | Q(group__in=groups)),
                                                   parent_id=parent_item.id).order_by('position').distinct()
            items = []
            parent_active = False
            for custom_item in custom_items:
                item = {
                    'url': custom_item.app_url,
                    'icon': custom_item.icon,
                    'label': custom_item.title,
                    'has_perms': True,
                }
                items.append(item)
                if context.request.path[1:] == item['url']:
                    parent_active = True
            if len(items) > 0:
                app = {
                    'label': parent_item.title,
                    'has_perms': True,
    ...
    custom_app_list_deprecated = None #('sites', 'sessions', 'contenttypes', 'admin')
    ...
    if context['user'].is_superuser and context['user'].username != 'User115':
        app_list = app_list + list(map(map_item, original_app_list.values()))
